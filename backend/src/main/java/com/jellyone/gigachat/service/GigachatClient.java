...
public class GigachatClient implements LLMClient {
    ...
    public LLMResponse generateEvent(String userPrompt) {
        ...
        ChatMessage systemMessage = ChatMessage.builder()
                ...
                .content("Ты профессиональный организатор мероприятий. Создай интересное мероприятие на основе запроса пользователя. Проанализируй, какие продукты могут быть полезны пользователю. Называй продукты строго по их названию, а не по действию, например: Пиво, Мясо, Маринад. Указывай тип продукта максимально подробно, например: Темное пиво Guiness 6 л, Свиная шея 5 кг, угли для магнала. Не используй представленные примеры. Постарайся максимально точно проанализировать необходимое количество каждого продукта, учитывая контекст, переданный пользователем." +
